...
export class ChannelsService {
  ...
  async banUser(channelId: number, bannedUserId: number, actingUserId: number): Promise<void> {
    ...
    if (
      actingUserRelation.isOwner ||
      (actingUserRelation.isAdmin && !bannedUserRelation.isAdmin && !bannedUserRelation.isOwner)
    ) {
  ...
  async inviteUser(
    ...
    if (existingInvitation && existingInvitation.status === InvitationStatus.Refused) {
      existingInvitation = this.channelInvitationRepository.merge(existingInvitation, {
  ...
  async muteUser(channelId: number, mutedUserId: number, actingUserId: number): Promise<void> {
    ...
    if (
      actingUserRelation.isOwner ||
      (actingUserRelation.isAdmin && !userToMuteRelation.isAdmin && !userToMuteRelation.isOwner)
    ) {
      const now = new Date();
